...
ErroCode GetVideoThread::init(QString videoDevName, bool useDesk, bool useCam, QString audioDevName, bool useAudio)
{
    ...
    }else if(useCam)
    {
        ...
        char* buf = (char*)str.c_str();
        lastReadPacktTime = av_gettime();
        if(avformat_open_input(&pFormatCtx,buf,ifmt1,/*&d*/NULL)!=0){
            qDebug()<<"打开另一个视频流";
    ...
    if (useDesk||useCam)
    {
        ...
        avpicture_fill((AVPicture *)pFrameYUV, out_buffer, AV_PIX_FMT_YUV420P, pCodecCtx->width, pCodecCtx->height);//挂buffer
