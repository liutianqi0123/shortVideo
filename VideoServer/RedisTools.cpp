...
vector<string> RedisTool::HashKeysContent(string key)
{
    if(m_redis == NULL || m_redis->err)
    {
        cout << "Redis init Error !!!" << endl;
...
// hmget
// key ,list<string> fields , list<string> vals

// string str;
// str = "hmget " + key + " ";
// while(fields.size() )
// {
//     str = str + " "+fields.at() + " "+vals.at() ;
// }
// redisCommand(m_redis, str.c_str() );

//hvals

vector<string> RedisTool::HashValsContent(string key)
{
    if(m_redis == NULL || m_redis->err)
    {
        cout << "Redis init Error !!!" << endl;
    ...
    redisReply *reply;
    reply = (redisReply *)redisCommand(m_redis,"hvals %s", key.c_str() );
    //cout<<"get list type = "<<reply->type<<endl;
    //cout<<"get list size = "<<reply->elements<<endl;//对于数组类型可以用elements元素获取数组长度
...
//目前只写了添加一个元素，后续完善
void RedisTool::ZaddZsetValue(string key, string value, string field)
{
    if(m_redis == NULL || m_redis->err)
    {
        cout << "Redis init Error !!!" << endl;
    ...
    {
        reply = (redisReply*)redisCommand(m_redis,"zadd %s %s %s", key.c_str(),value.c_str() ,field.c_str());
    }

    freeReplyObject(reply);
    cout<<"Zadd zset success"<<endl;
    return;
}

vector<string> RedisTool::ZRangeZSetKeys(string key, int left, int right)
{
    if(m_redis == NULL || m_redis->err)
    {
        cout << "Redis init Error !!!" << endl;
    ...
    redisReply *reply;
    reply = (redisReply *)redisCommand(m_redis,"zrange %s %d %d" , key.c_str(),left,right );
    //cout<<"get list type = "<<reply->type<<endl;
    //cout<<"get list size = "<<reply->elements<<endl;//对于数组类型可以用elements元素获取数组长度
